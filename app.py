...
def calculate_confidence(data):
    ...
    try:
        ...
        print(f"SMA5: {sma5}, SMA20: {sma20}")
        if pd.notna(sma5) and pd.notna(sma20) and sma5 > sma20:
            confidence += 1
    ...
    try:
        ...
        print(f"RSI raw value: {rsi_value}")
        if isinstance(rsi_value, (np.ndarray, pd.Series)):
            rsi_value = rsi_value.values[0]
    ...
    try:
        ...
        print(f"MACD: {macd_val}, Signal: {signal_val}")
        if isinstance(macd_val, (np.ndarray, pd.Series)):
            macd_val = macd_val.values[0]
        if isinstance(signal_val, (np.ndarray, pd.Series)):
            signal_val = signal_val.values[0]
        if pd.notna(macd_val) and pd.notna(signal_val) and float(macd_val) > float(signal_val):
            confidence += 1
    except Exception as e:
        print("MACD error:", e)

    print(f"Confidence score: {confidence}/{total_indicators}")
